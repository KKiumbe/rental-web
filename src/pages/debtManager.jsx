...
function DebtManager() {
  ...
  const [balanceThreshold, setBalanceThreshold] = useState(""); // Input for balance threshold
  const [customBalanceThreshold, setCustomBalanceThreshold] = useState(""); // Input for custom message balance threshold
  ...
  // 2. Send SMS to Customers Above Selected Balance (Input)
  const handleSendSmsAboveBalance = async () => {
    ...
    try {
      const response = await axios.post(
        `${BASEURL}/send-sms-above-balance`,
        { balance },
        { withCredentials: true }
      );
      setMessage(response.data.message || `SMS sent to customers with balance above ${balance}`);
      setBalanceThreshold(""); // Clear input
      setSnackbarOpen(false);
    } catch (error) {
      const errorMsg = error.response?.data?.message || "Error sending SMS";
      setMessage("");
  ...
  // 3. Send Custom SMS to Customers Above Selected Balance (Input)
  const handleSendCustomSms = async () => {
    ...
    try {
      ...
      setMessage(response.data.message || `Custom SMS sent to customers with balance above ${balance}`);
      setCustomBalanceThreshold(""); // Clear input
    ...
    <Box sx={{ width: "100vw", minHeight: "100vh", display: "flex", justifyContent: "center", alignItems: "center" }}>
      <Container>
        ...
        <Paper elevation={3} sx={{ maxWidth: '60%', bgcolor: theme.palette.background.paper }}>
          <Box sx={{ p: 3 }}>
            {/* Section 1: Send SMS to All with Arrears */}
            <Box sx={{ mb: 4 }}>
            ...
            {/* Section 2: Send SMS to Customers Above Selected Balance (Input) */}
            <Box sx={{ mb: 4 }}>
              <Typography variant="h6" gutterBottom sx={{ color: theme.palette.grey[100] }}>
            ...
            {/* Section 3: Send Custom SMS to Customers Above Selected Balance (Input) */}
            <Box sx={{ mb: 4 }}>
              <Typography variant="h6" gutterBottom sx={{ color: theme.palette.grey[100] }}>
