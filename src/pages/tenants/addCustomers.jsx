...
export default function CreateCustomerScreen() {
  ...
  const fetchBuildings = async () => {
    try {
      ...
      console.log(`buildings: ${JSON.stringify(buildingsData, null, 2)}`);
    } catch (error) {
      console.error('Error fetching buildings:', error);
      setSnackbarMessage(error.response?.data?.message || 'Failed to load buildings. Please try again.');
  ...
  const fetchUnits = async (buildingId) => {
    try {
      ...
      setUnits([]); // Clear units while loading
      ...
      if (fetchedUnits.length === 0) {
        setSnackbarMessage('No units available for the selected building.');
        setSnackbarSeverity('warning');
        setSnackbarOpen(true);
      }
    } catch (error) {
      console.error('Error fetching units:', error);
      setSnackbarMessage(error.response?.data?.message || 'Failed to load units. Please try again.');
      setSnackbarSeverity('error');
      setSnackbarOpen(true);
      setUnits([]);
    } finally {
  ...
  // Handle unit selection for single customer
      ...
      <Alert severity="info" sx={{ mb: 2 }}>
        <Typography variant="body2" component="div">
          ...
          <ul>
            ...
            <li>
              <strong>firstName</strong> (Optional): Customer's first name. At least one of firstName or lastName is required.
            </li>
            <li>
              <strong>lastName</strong> (Optional): Customer's last name. If firstName is missing, lastName is used for both.
            </li>
            <li>
  ...
  // Other form render functions (unchanged for brevity)
